...
interface JobListingRow {
  ...
  updatedAt: string;
  currentVersion: { translations: Record<"en" | "ar", { title: string; team: string }>; datePosted: string | null } | null;
}

type LoadState = { kind: "loading" } | { kind: "forbidden" } | { kind: "error" } | { kind: "ready"; listings: JobListingRow[] };
...
export default function JobListingListPage() {
  ...
  const load = useCallback(async (status: string) => {
    try {
      const url = status ? `/api/admin/careers?status=${status}` : "/api/admin/careers";
      ...
      if (sessionRes.ok) {
        ...
        setCanCreate(perms.has("careers:create"));
          ...
          <table className="w-full text-start text-sm">
            <thead className="border-b border-neutral-200 bg-neutral-50 text-xs font-medium uppercase tracking-wide text-neutral-500">
              <tr>
                ...
                <th className="px-4 py-3 text-start">{t("teamHeader")}</th>
            ...
            <tbody className="divide-y divide-neutral-100">
              {state.listings.map((row) => (
                <tr key={row.id}>
                  <td className="px-4 py-3 font-medium text-neutral-800">{row.currentVersion?.translations[locale]?.title || row.currentVersion?.translations.en.title || row.slug}</td>
                  <td className="px-4 py-3 text-neutral-600">{row.currentVersion?.translations[locale]?.team ?? row.currentVersion?.translations.en.team ?? "—"}</td>
                  ...
                  <td className="px-4 py-3">
                    <div className="flex items-center gap-1">
                      <ActionButton href={`/careers/${row.id}`}>{t("edit")}</ActionButton>
                      {canDelete ? (
