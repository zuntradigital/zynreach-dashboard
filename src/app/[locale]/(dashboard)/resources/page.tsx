...
interface ResourceRow {
  ...
  updatedAt: string;
  currentVersion: { translations: Record<"en" | "ar", { title: string }> } | null;
}

type LoadState = { kind: "loading" } | { kind: "forbidden" } | { kind: "error" } | { kind: "ready"; resources: ResourceRow[] };

const STATUS_OPTIONS = ["DRAFT", "SUBMITTED", "IN_REVIEW", "CHANGES_REQUESTED", "APPROVED", "SCHEDULED", "PUBLISHED", "ARCHIVED"] as const;
const FORMAT_OPTIONS = ["GUIDE", "TEMPLATE", "WHITEPAPER", "CHECKLIST", "PLAYBOOK"] as const;
  ...
  const load = useCallback(async (status: string) => {
    try {
      ...
      if (sessionRes.ok) {
        ...
        setCanCreate(new Set<string>(sessionData.user.permissions).has("resources:create"));
      }
      setState({ kind: "ready", resources: data.resources });
    } catch {
      setState({ kind: "error" });
    }
  }, []);
          ...
          <table className="w-full text-start text-sm">
            <thead className="border-b border-neutral-200 bg-neutral-50 text-xs font-medium uppercase tracking-wide text-neutral-500">
              <tr>
                ...
                <th className="px-4 py-3 text-start">{t("formatHeader")}</th>
            ...
            <tbody className="divide-y divide-neutral-100">
              {state.resources.map((row) => (
                <tr key={row.id}>
                  <td className="px-4 py-3 font-medium text-neutral-800">{row.currentVersion?.translations.en.title || row.slug}</td>
                  <td className="px-4 py-3 text-neutral-600">{t(`format.${row.resourceFormat}`)}</td>
                  ...
                  <td className="px-4 py-3">
                    <ActionButton href={`/resources/${row.id}`}>{t("edit")}</ActionButton>
                  </td>
                </tr>
              ))}
            </tbody>
...
function CreateResourceModal({ onClose, onCreated }: { onClose: () => void; onCreated: (id: string) => void }) {
  const t = useTranslations("dashboard.resources");
  const [slug, setSlug] = useState("");
  const [resourceFormat, setResourceFormat] = useState<(typeof FORMAT_OPTIONS)[number]>("GUIDE");
  ...
  async function handleSubmit(event: FormEvent) {
    ...
    try {
      const res = await fetch("/api/admin/resources", {
        ...
        body: JSON.stringify({ slug, resourceFormat }),
      });
      if (res.status === 409) {
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-neutral-900/40 px-4">
      <div className="w-full max-w-md rounded-lg bg-surface p-6 shadow-lg">
        ...
        <form onSubmit={handleSubmit} className="mt-4 space-y-4" noValidate>
          ...
          <div>
            <label className="block text-sm font-medium text-neutral-700">{t("formatLabel")}</label>
            <select
              value={resourceFormat}
              onChange={(e) => setResourceFormat(e.target.value as (typeof FORMAT_OPTIONS)[number])}
              className="mt-1.5 block min-h-9 w-full rounded-md border border-neutral-300 bg-surface px-2 text-sm text-neutral-900"
            >
