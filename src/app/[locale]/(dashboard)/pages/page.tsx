...
type LoadState = { kind: "loading" } | { kind: "forbidden" } | { kind: "error" } | { kind: "ready"; pages: PageRow[] };
...
export default function PagesListPage() {
  ...
  const load = useCallback(async (status: string) => {
    try {
      const url = status ? `/api/admin/pages?status=${status}` : "/api/admin/pages";
      ...
      if (sessionRes.ok) {
        ...
        setCanCreate(new Set<string>(sessionData.user.permissions).has("content:create"));
      }
      setState({ kind: "ready", pages: data.pages });
    } catch {
      setState({ kind: "error" });
    }
  }, []);
          ...
          <table className="w-full text-start text-sm">
            <thead className="border-b border-neutral-200 bg-neutral-50 text-xs font-medium uppercase tracking-wide text-neutral-500">
              <tr>
                ...
                <th className="px-4 py-3 text-start">{t("slugHeader")}</th>
                <th className="px-4 py-3 text-start">{t("statusHeader")}</th>
                <th className="px-4 py-3 text-start">{t("classificationHeader")}</th>
            ...
            <tbody className="divide-y divide-neutral-100">
              {state.pages.map((row) => (
                <tr key={row.id}>
                  <td className="px-4 py-3 font-medium text-neutral-800">{row.title}</td>
                  ...
                  <td className="px-4 py-3 text-neutral-600">{t(`classification.${row.classification}`)}</td>
                  <td className="px-4 py-3 text-neutral-500">{new Date(row.updatedAt).toLocaleString()}</td>
                  <td className="px-4 py-3">
                    <ActionButton href={`/pages/${row.id}`}>{t("edit")}</ActionButton>
                  </td>
                </tr>
              ))}
            </tbody>
...
function CreatePageModal({ onClose, onCreated }: { onClose: () => void; onCreated: (id: string) => void }) {
  ...
  const [classification, setClassification] = useState<"STANDARD" | "PRICING" | "LEGAL" | "SECURITY">("STANDARD");
  ...
  async function handleSubmit(event: FormEvent) {
    ...
    try {
      const res = await fetch("/api/admin/pages", {
        ...
        body: JSON.stringify({ slug, title, templateType, classification, componentBlocks: [] }),
      });
      if (res.status === 409) {
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-neutral-900/40 px-4">
      <div className="w-full max-w-md rounded-lg bg-surface p-6 shadow-lg">
        ...
        <form onSubmit={handleSubmit} className="mt-4 space-y-4" noValidate>
          <TextField label={t("titleLabel")} name="title" value={title} onChange={setTitle} required autoFocus />
          <div>
          ...
          <TextField label={t("templateTypeLabel")} name="templateType" value={templateType} onChange={setTemplateType} />
          ...
          <div>
            ...
            >
              {(["STANDARD", "PRICING", "LEGAL", "SECURITY"] as const).map((c) => (
